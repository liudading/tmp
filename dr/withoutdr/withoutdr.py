...
def scheduling(env, plot=False):
    ...
    P_ev, SOC = [0.0] * T_ev, [0.0] * T_ev
    for t in range(T_ev):
        if t == 0:
            P_ev[t] = min(env.ev.MAX_POWER, (env.ev.MAX_SOC - env.ev.init_soc) * env.ev.CAPACITY / (env.dt.seconds/3600))
            SOC[t] = env.ev.init_soc + P_ev[t] * (env.dt.seconds/3600) / env.ev.CAPACITY
        else:
            P_ev[t] = min(env.ev.MAX_POWER, (env.ev.MAX_SOC - SOC[t-1]) * env.ev.CAPACITY / (env.dt.seconds/3600))
            SOC[t] = SOC[t-1] + P_ev[t] * (env.dt.seconds/3600) / env.ev.CAPACITY

    # ewh
    P_wh, T_wh = [0.0] * env.T, [0.0] * env.T
    for t in range(env.T):
        ...
        # if t == 0:
        #     P_wh[t] = min(env.wh.MAX_POWER, ((env.wh.MAX_TEMP-coff*sum(env.wh.state[1:]))/(1-coff)-G*R1*T_air-B*R1*Tin)/((env.dt.seconds/3600)*Q*R1))
        #     T_wh[t] = coff*sum(env.wh.state[1:])+(1-coff)*(G*R1*T_air+B*R1*Tin+P_wh[t]*(env.dt.seconds/3600)*Q*R1)
        # else:
        #     P_wh[t] = min(env.wh.MAX_POWER, ((env.wh.MAX_TEMP-coff*T_wh[t-1])/(1-coff)-G*R1*T_air-B*R1*Tin)/((env.dt.seconds/3600)*Q*R1))
        #     T_wh[t] = coff*T_wh[t-1]+(1-coff)*(G*R1*T_air+B*R1*Tin+P_wh[t]*(env.dt.seconds/3600)*Q*R1)

        if t == 0:
            if sum(env.wh.state[1:]) < env.wh.MIN_TEMP:
                P_wh[t] = env.wh.MAX_POWER
            elif sum(env.wh.state[1:]) > env.wh.MAX_TEMP:
                P_wh[t] = env.wh.MIN_POWER
            else:
                P_wh[t] = env.wh.MIN_POWER
            T_wh[t] = coff*sum(env.wh.state[1:])+(1-coff)*(G*R1*T_air+B*R1*Tin+P_wh[t]*(env.dt.seconds/3600)*Q*R1)
        else:
            if T_wh[t-1] < env.wh.MIN_TEMP:
                P_wh[t] = env.wh.MAX_POWER
            elif T_wh[t-1] > env.wh.MAX_TEMP:
                P_wh[t] = env.wh.MIN_POWER
            else:
                P_wh[t] = P_wh[t-1]
            T_wh[t] = coff*T_wh[t-1]+(1-coff)*(G*R1*T_air+B*R1*Tin+P_wh[t]*(env.dt.seconds/3600)*Q*R1)
    ...
    P_ac, T_ac = [0.0] * env.T, [0.0] * env.T
    for t in range(env.T):
        if t == 0:
            if sum(env.ac.state[1:]) > env.ac.MAX_TEMP:
                P_ac[t] = env.ac.MAX_POWER
            elif sum(env.ac.state[1:]) < env.ac.MIN_TEMP:
                P_ac[t] = env.ac.MIN_POWER
            else:
                P_ac[t] = env.ac.MIN_POWER
            T_ac[t] = (1-delta_t/(1000*Ma*Ca*Req))*sum(env.ac.state[1:]) + delta_t/(1000*Ma*Ca*Req)*temp[t] - (P_ac[t]*COP*delta_t)/(0.00027*Ma*Ca)
        else:
            if T_ac[t-1] > env.ac.MAX_TEMP:
                P_ac[t] = env.ac.MAX_POWER
            elif T_ac[t-1] < env.ac.MIN_TEMP:
                P_ac[t] = env.ac.MIN_POWER
            else:
                P_ac[t] = P_ac[t-1]
            T_ac[t] = (1-delta_t/(1000*Ma*Ca*Req))*T_ac[t-1]+ delta_t/(1000*Ma*Ca*Req)*temp[t] - (P_ac[t]*COP*delta_t)/(0.00027*Ma*Ca)
        # if t == 0:
        #     P_ac[t] = min(
        #         env.ac.MAX_POWER, 
        #         ((1-delta_t/(1000*Ma*Ca*Req))*sum(env.ac.state[1:]) + delta_t/(1000*Ma*Ca*Req)*temp[t] - env.ac.MIN_TEMP)*(0.00027*Ma*Ca)/(COP*delta_t)
        #     )
        #     P_ac[t] = max(0, P_ac[t])
        #     T_ac[t] = (1-delta_t/(1000*Ma*Ca*Req))*sum(env.ac.state[1:]) + delta_t/(1000*Ma*Ca*Req)*temp[t] - (P_ac[t]*COP*delta_t)/(0.00027*Ma*Ca)
        # else:
        #     P_ac[t] = min(
        #         env.ac.MAX_POWER, 
        #         ((1-delta_t/(1000*Ma*Ca*Req))*T_ac[t-1]+delta_t/(1000*Ma*Ca*Req)*temp[t]-env.ac.MIN_TEMP)*(0.00027*Ma*Ca)/(COP*delta_t)
        #     )
        #     P_ac[t] = max(0, P_ac[t])
        #     T_ac[t] = (1-delta_t/(1000*Ma*Ca*Req))*T_ac[t-1] + delta_t/(1000*Ma*Ca*Req)*temp[t] - (P_ac[t]*COP*delta_t)/(0.00027*Ma*Ca)
    ...
    p_ov = np.hstack([
        np.zeros(ta_ov), 
        [B_ov[t]*env.ov.MAX_POWER for t in range(T_ov)], 
        np.zeros(env.T-tb_ov)
        ])
    p_dw = np.hstack([
        np.zeros(ta_dw), 
        [B_dw[t]*env.dw.MAX_POWER for t in range(T_dw)], 
        np.zeros(env.T-tb_dw)
        ])
    p_wm = np.hstack([
        np.zeros(ta_wm),
        [B_wm[t]*env.wm.MAX_POWER for t in range(T_wm)], 
        np.zeros(env.T-tb_wm)
        ])
    p_cd = np.hstack([
        np.zeros(ta_cd),
        [B_cd[t]*env.cd.MAX_POWER for t in range(T_cd)], 
    ...
    p_ev = np.hstack([
        np.zeros(ta_ev),
        [P_ev[t] for t in range(T_ev)], 
        np.zeros(env.T-tb_ev)
        ])
    soc = np.hstack([
        np.zeros(ta_ev-1),
        [env.ev.init_soc]+[SOC[t] for t in range(T_ev)], 
        np.zeros(env.T-tb_ev)
        ])
    p_wh = np.array(P_wh)
    temp_wh = np.array(T_wh)
    p_ac = np.array(P_ac)
    temp_ac = np.array(T_ac)
    f = sum(OBJ)
