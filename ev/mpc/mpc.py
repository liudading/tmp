...
def model_predict(arr_date, cur_time, dep_time, price, soc, rnd):
    while True:
        ...
        if dep_time > cur_time:
            break
    ep_prices = price.loc[cur_time:dep_time-pd.Timedelta(hours=1)]["PRICE"].values
    T = ep_prices.size
    # predict_ep_prices = rnd.normal(ep_prices, ep_prices*np.arange(0.1,0.1+0.01*T,0.01)[:T])
    predict_ep_prices = rnd.normal(ep_prices, ep_prices*0.1)
    f, p, soc = optimize(predict_ep_prices, soc)
    return p[0], soc[0]
...
for arr_date in price['DATE'].unique()[1:-1]:
    ...
    soc = init_soc = np.clip(rnd.normal(0.5, 0.1), 0.2, 0.8)
    ep_p, ep_soc = [], [soc]
    while cur_time != dep_time:
        p, soc = model_predict(arr_date, cur_time, dep_time, price, soc, rnd)
        ep_p.append(p)
        ep_soc.append(soc)
        cur_time += pd.Timedelta(hours=1)
    f = np.array(ep_p).dot( ep_prices )
    s = abs(TARGET_SOC - ep_soc[-1]) * CAPACITY
    f_vals.append(f)
    s_vals.append(s)
    # plot(ep_prices, ep_soc)
    print(arr_date)
...
np.savetxt('/home/lihepeng/Documents/Github/tmp/ev/mpc/mpc_returns.txt', f_vals)
np.savetxt('/home/lihepeng/Documents/Github/tmp/ev/mpc/mpc_safeties.txt', s_vals)